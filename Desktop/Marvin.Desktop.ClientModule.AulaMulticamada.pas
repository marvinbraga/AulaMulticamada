...
begin
  ...
  LJSONObject := TJSONObject.ParseJSONValue(TEncoding.ASCII.GetBytes(LEntrada),
    0) as TJSONObject;
  try
    { recupera o result }
...
begin
  { transforma a string em objeto JSON }
  LJSONObject := TJSONObject.ParseJSONValue(TEncoding.ASCII.GetBytes(AJSONString),
    0) as TJSONObject;
  try
    { recupera o result }
    LArrayResult := LJSONObject.GetValue<TJSONArray>('result');
